---
title: "Germline SNVs in JHU Biobank Batch 1,2, and 3"
author:
...
```{python login_to_synapse, engine.path="/usr/local/bin/python", eval=TRUE, echo=FALSE, results='hide'}
...
Raw fastq data files were quality checked using FastQC v0.11.9 and a report was generated using MultiQC      v1.8. Fastq files were aligned to GRCh38 using BWA   0.7.17. Duplicates were marked using GATK MarkDuplicates, and bases recalibrated using GATK BaseRecalibrator and GATK ApplyBQSR      (GATK v4.1.7.0). Germline variants were then called using Google's DeepVariant software (deepvariant v1.1.0). The variants were annotated using Variant Effect Predictor (VEP  v99.2) and converted to MAF files using vcf2maf (vcf2maf v1.6.21). All of these steps were completed on Nextflow Tower running the standardized nf-core pipeline [sarek v2.7.1](https://nf-co.re/sarek/2.7.1).

### Sample Summary

```{python clinical_data_from_tables, engine.path="/usr/local/bin/python", echo=FALSE, eval=TRUE, results='hide'}
...
mpnst_clindata <- jhu_triplicates_clinicaldata %>% 
  dplyr::filter(tumorType == "Malignant Peripheral Nerve Sheath Tumor")
#add filepath
mpnst_clindata$path <- paste0("/home/rstudio/data/biobank-release-2/data/germline_mafs/", mpnst_clindata$Tumor_Sample_Barcode, ".maf")

pnf_clindata <- jhu_triplicates_clinicaldata %>% 
  dplyr::filter(tumorType == "Plexiform Neurofibroma")
#add filepath
pnf_clindata$path <- paste0("/home/rstudio/data/biobank-release-2/data/germline_mafs/", pnf_clindata$Tumor_Sample_Barcode, ".maf")

normal_clindata <- jhu_triplicates_clinicaldata %>% 
  dplyr::filter(tumorType == "Not Applicable")

cnf_clindata <- jhu_triplicates_clinicaldata %>% 
  dplyr::filter(tumorType == "Cutaneous Neurofibroma" |
                  tumorType == "Diffuse Infiltrating Neurofibroma" |
                  tumorType == "Neurofibroma with Degenerative Atypia" |
...
files <- (Sys.glob("/home/rstudio/data/biobank-release-2/data/germline_mafs/*.maf"))

mpnst_clindata <- mpnst_clindata %>% 
  dplyr::filter(mpnst_clindata$path %in% files)
listOfFiles_mpnst <- lapply(mpnst_clindata$path, function(x) maftools::read.maf(maf = x,
                                                            clinicalData = mpnst_clindata))

pnf_clindata <- pnf_clindata %>% 
  dplyr::filter(pnf_clindata$path %in% files)
listOfFiles_pnf <- lapply(pnf_clindata$path, function(x) maftools::read.maf(maf = x,
                                                            clinicalData = pnf_clindata))

normal_clindata <- normal_clindata %>% 
  dplyr::filter(normal_clindata$path %in% files)
listOfFiles_normal <- lapply(files, function(x) maftools::read.maf(maf = x,
                                                            clinicalData = normal_clindata))

cnf_clindata <- cnf_clindata %>% 
  dplyr::filter(cnf_clindata$path %in% files)
listOfFiles_cnf <- lapply(files, function(x) maftools::read.maf(maf = x,
                                                            clinicalData = cnf_clindata))

mpnst_snv <- maftools::merge_mafs(listOfFiles_mpnst)
pnf_snv <- maftools::merge_mafs(listOfFiles_pnf)
normal_snv <- maftools::merge_mafs(listOfFiles_normal)
cnf_snv <- maftools::merge_mafs(listOfFiles_cnf)
...
### Visualizing single nucleotide variants in PNFs and MPNSTs from all batches

Germline calls from Deepvariant were visualized in the all the samples without any prefilter steps. The samples include tumor samples from 3 different sequencing batches.
...
#![Figure 1](./experiments/figures/Germline_all_samples_unfiltered_summary.png "figure 1")

# pdf("/Users/jineta/git/gitrepo/biobank-release-2/experiments/figures/Somatic_all_samples_unfiltered_summary.pdf", height = 20, width = 20)
...
pdf("./experiments/figures/Germline_pnf_mpnst_samples_unfiltered_GOI.pdf", height = 40, width = 40)
...
![Figure 2](./experiments/figures/Germline_pnf_mpnst_samples_unfiltered_GOI.png "shows an oncoplot of all unfiltered variants identified in our list of genes of interest.") 
...
#PASS filter
#PASS filter
pnf_maf@data <- pnf_maf@data %>% dplyr::filter(pnf_maf@data$FILTER != "common_variant" & 
                                                     pnf_maf@data$FILTER != "RefCall" & 
                                                     pnf_maf@data$FILTER != "RefCall;common_variant" & 
                                                     pnf_maf@data$IMPACT == "HIGH")
...
![Figure 3](/Users/jineta/git/gitrepo/biobank-release-2/experiments/figures/Somatic_pnf_mpnst_passfiltered_top.png "shows the filtered variants in top 500 mutated genes in PNF and MPNST tumor samples.") 
![Figure 4](/Users/jineta/git/gitrepo/biobank-release-2/experiments/figures/Somatic_pnf_mpnst_passfiltered_GOI.png "shows filtered variants in our list of genes of interest.")
...
# pdf("/Users/jineta/git/gitrepo/biobank-release-2/experiments/figures/Somatic_pnf_mpnst_passfiltered_top.pdf", height = 40, width = 40)
...
# pdf("/Users/jineta/git/gitrepo/biobank-release-2/experiments/figures/Somatic_pnf_mpnst_passfiltered_GOI.pdf", height = 40, width = 40)
...
![Figure 5](/Users/jineta/git/gitrepo/biobank-release-2/experiments/figures/Somatic_triad_pnf_mpnst_passfiltered_top.png "shows the filtered variants in top 500 mutated genes in PNF and MPNST tumors in triads. samples.") 
![Figure 6](/Users/jineta/git/gitrepo/biobank-release-2/experiments/figures/Somatic_triad_pnf_mpnst_samples_passfiltered_GOI.png "shows filtered variants in our list of genes of interest in the triads.")

```{r co-oncoplot-triad, eval=TRUE, echo=FALSE, results='hide', message=FALSE, warning=FALSE, fig.height= 40, fig.width=40}

maf_file <- maftools::merge_mafs(list(mpnst_maf, pnf_maf))
# pdf("/Users/jineta/git/gitrepo/biobank-release-2/experiments/figures/Somatic_triad_pnf_mpnst_passfiltered_top.pdf", height = 40, width = 40)
# oncoplot(maf_file, top = 100,
#          altered = TRUE,
#          drawRowBar = FALSE, showTumorSampleBarcodes = TRUE, sortByAnnotation = TRUE,
#          clinicalFeatures = c("tumorType","individualID"),
...
# dev.off()
# 
# pdf("/Users/jineta/git/gitrepo/biobank-release-2/experiments/figures/Somatic_triad_pnf_mpnst_samples_passfiltered_GOI.pdf", height = 40, width = 40)
# maftools::coOncoplot(m1 = pnf_maf,
#                      m2 = mpnst_maf,
#                      m1Name = "Plexiform Neurofibroma",
#                      m2Name = "MPNST",
