...
Process_data=file.path("/Volumes/lyu.yang/MAC_1/R/R backup/Project/JH_PAPER","data","Process_data")
COUNT_path=file.path("/Volumes/lyu.yang/MAC_1/R/R backup/Project/JH_PAPER","data","RNAseq","Count")
...
```{r colors, echo=F, eval=TRUE, results='hide', message=FALSE, warning=FALSE}
...
PN=MPNST_PN_RNAseq_anno%>%dplyr::filter(Disease%in%c("PN"))
MPNST=MPNST_PN_RNAseq_anno%>%dplyr::filter(Disease%in%c("MPNST"))
...
# Power calculation
**Sample size calculation for gene-specific mean and dispersion with different fold change**
...
```{r}
all_counts=read.csv(file.path(COUNT_path,"51_JH_Tumor_raw_counts_for_paper.csv"),row.names = 1)
colnames(all_counts)=gsub("\\.","-",colnames(all_counts))
counts=all_counts[,PN$Sample_name]
# data(hammer.eset)
# counts <- exprs(hammer.eset)[, phenoData(hammer.eset)$Time == "2 weeks"]
counts <- counts[rowSums(counts) > 10,] ## filter zero count genes
...
```{r}
size2 <- ssizeRNA_vary(nGenes = 12000, pi0 = 0.8, m = 200, mu = mu,disp = disp, fc = 2, fdr = 0.05,power = 0.8, maxN = 40, replace = FALSE)
...
```{r}
size2$ssize
check.power(m = 14, mu = mu, disp = disp, fc = 2, sims = 100,replace = FALSE)
```
...
size3 <- ssizeRNA_vary(nGenes = 10000, pi0 = 0.8, m = 200, mu = mu,disp = disp, fc = fc, fdr = 0.05,
power = 0.8, maxN = 30, replace = FALSE)
